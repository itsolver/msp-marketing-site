...
(async () => {
  ...
  const intervalLowerCased = store.plans[0].interval + 'ly Payment';
  const interval = intervalLowerCased.slice(0,1).toUpperCase()+intervalLowerCased.slice(1)
  ...
  paymentRequest.on('source', async event => {
    try {
      ...
      const name = event.shippingAddress.recipient;
      ...
      const phone = event.shippingAddress.phone;
      const info = {
        phone,
      }
      console.info('event',event);
      console.info('event.source',event.source);
      // const {source} = await stripe.createSource(event.source.card, {
      //   owner: {
      //     name,
      //   },
      // });

      console.info('name, email, shipping, phone, info', name, email, shipping, phone, info);
      await handleSubscription(email, event.source, shipping, info);
      event.complete('success');
    } catch (error) {
      console.error('Payment Request fail', error);
      event.complete('fail');
    }
  });
    ...
    switch (order.metadata.status) {
      ...
      case 'pending':
        ...
        confirmationElement.querySelector('.note').innerText =
          'We’ll send your receipt and book in the service as soon as your payment is confirmed.';
        mainElement.classList.add('success');
        break;
      ...
      case 'paid':
        ...
        confirmationElement.querySelector('.note').innerText =
          'We just sent your receipt to your email address, and you will now be asked to book in the service.';
        mainElement.classList.add('success');
        break;
    }
  };
